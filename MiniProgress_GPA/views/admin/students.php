...
<h2 class="page-title"><?= t('students') ?></h2>
...
<section class="panel">
    <h3><?= $edit ? (currentLang()==='ar'?'تعديل الطالب':'Edit Student') : (currentLang()==='ar'?'إضافة طالب':'Add Student') ?></h3>
    <form class="form-grid" method="POST" action="<?= pageUrl('admin.saveStudent') ?>">
        ...
        <div>
            ...
            <?php if ($edit): ?>
            <a href="<?= pageUrl('admin.students') ?>" class="btn btn-secondary"><?= currentLang()==='ar'?'إلغاء':'Cancel' ?></a>
            <?php endif; ?>
        </div>
    </form>
</section>
...
<div class="table-wrap">
    <table>
        ...
        <tr>
            ...
            <td class="actions">
                <a href="<?= pageUrl('admin.students', ['edit' => $s['id']]) ?>" class="btn btn-secondary btn-sm"><?= currentLang()==='ar'?'تعديل':'Edit' ?></a>
                <form method="POST" action="<?= pageUrl('admin.deleteStudent') ?>" style="display:inline;" onsubmit="return confirm('Delete?')">
...
<?php for ($i = 1; $i <= $pages; $i++): ?>
    <li class="page-item <?= $i === ($page ?? 1) ? 'active' : '' ?>">
        <a class="page-link" href="<?= pageUrl('admin.students', ['p' => $i, 'q' => $search ?? '']) ?>"><?= $i ?></a>
    </li>
<?php endfor; ?>
</ul></nav>
